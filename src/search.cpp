        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, ttData.move,
                                       std::min(130 * depth - 71, 1043), PvNode);
...
void update_all_stats(const Position& pos,
    ...
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * 957 / 1024, PvNode);
...
void update_quiet_histories(const Position& pos,
    ...
    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(),
                                  bonus * ((bonus > 0 && PvNode) ? 1024 : 793) / 1024);

    int pIndex = pawn_history_index(pos);
    workerThread.pawnHistory[pIndex][pos.moved_piece(move)][move.to_sq()]
      << (bonus * (bonus > 0 ? (PvNode? 704: 650) : (PvNode? 439: 462)) / 1024) + 70;
}
